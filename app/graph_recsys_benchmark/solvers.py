...
from app.graph_recsys_benchmark.utils import *
...
class BaseSolver(object):
    ...
    def metrics(
        ...
        for u_idx, u_nid in enumerate(test_bar):
            ...
            pos_u_nids_t = torch.from_numpy(np.array([u_nid for _ in range(len(pos_i_nids))])).to(self.device)
            pos_i_nids_t = torch.from_numpy(np.array(pos_i_nids)).to(self.device)
            neg_u_nids_t = torch.from_numpy(np.array([u_nid for _ in range(len(neg_i_nids))])).to(self.device)
            neg_i_nids_t = torch.from_numpy(np.array(neg_i_nids)).to(self.device)
            if self.model_args['model_type'] == 'MF':
